...
<head>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function () {
            $("#grid").jqGrid({
                url: '/manage/managerManage/load',//请求数据的地址
                mtype: "POST",
                datatype: "json",
                ...
                colModel: [
                    ...
                    {
                        name: 'name', index: 'name', width: 200, align: 'center',editable: true,
                        editoptions: {size: "20", maxlength: "30"}
                    },

                    {name: 'tel', index: 'tel', align: 'center',width: 230, editable: true},
                    {name: 'email', index: 'email', align: 'center',width: 230},
                    {name: 'password', index: 'password',align: 'center', width: 0,hidden:true}
...
//            };
            ...
            if (flag == false) {
                ...
                $.ajax({
                    url: "${basePath}/manage/managerManage/updateManager",
                    data: params,
                    dataType: "json",
                    cache: false,
...
//          alert("id-->" + response.id + "; message-->" + response.message);
        ...
        var deleteItem = function () {
            ...
            $.ajax({
                url: "${basePath}/manage/managerManage/delete",
                data: params,
                dataType: "html",
                cache: false,
                success: function (id) {
                        ...
                        if (data != null) {
                            if (data.message != null) {
                                ...
                                $("#grid").jqGrid("addRowData",
                                    data.manager.id, dataRow, "last");    //将新行插入到末尾
                                jQuery("#grid").trigger("reloadGrid");
                                alert("添加成功!");
                                layer.close(index);
                            }
        ...
        var search = function () {
            ...
            $("#grid").jqGrid("setGridParam",
                {
                    url: "/manage/managerManage/search",
                    datatype: 'json',
                    postData: { //传递查询参数到后台
                        'name': name
                    },
